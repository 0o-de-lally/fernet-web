...
fn bench_complete_decrypt_workflow(c: &mut Criterion) {
    ...
    for scenario in scenarios.iter().take(2) { // Limit to first 2 scenarios for this benchmark
...
fn bench_error_handling_scenarios(c: &mut Criterion) {
    ...
    let error_scenarios = vec![
        ...
        ("very_long_rsa_key", "a_very_long_key_placeholder_for_benchmark"),
    ];
    
    for (name, key) in error_scenarios {
        group.bench_with_input(
            BenchmarkId::new("rsa_errors", name),
            &key,
            |b, key| {
                b.iter(|| {
    ...
    let fernet_scenarios = vec![
        ...
        ("large_token", ([0u8; 16], "very_large_token_placeholder_for_benchmark")), // Very large token with wrong key size
    ];
