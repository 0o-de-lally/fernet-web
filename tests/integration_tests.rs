...
async fn test_server_config_validation() {
    ...
    let config = create_test_config().await.expect("Failed to create test config");
...
async fn test_server_config_defaults() {
    ...
    assert_eq!(config.log_level, fernet_web::server::config::LogLevel::from(tracing::Level::INFO));
...
async fn test_crypto_service_initialization() {
    ...
    assert!(crypto_service.is_ok(), "Crypto service should initialize successfully");
    ...
    assert!(public_key.contains("BEGIN PUBLIC KEY"), "Should be in PEM format");
    
    // Test metrics
    let metrics = service.get_metrics();
    assert_eq!(metrics.rsa_operations, 0, "Should start with zero operations");
    assert_eq!(metrics.fernet_operations, 0, "Should start with zero operations");
}
...
async fn test_fernet_key_operations() {
    ...
    assert_eq!(key.get_key_string().len(), 44, "Base64 encoded key should be 44 chars");
    
    // Test round-trip conversion
    let recovered_bytes = key.get_key_bytes().expect("Should be able to get bytes back");
    assert_eq!(recovered_bytes, key_bytes, "Round-trip should preserve bytes");
    
    // Test invalid key sizes
    let short_key = [42u8; 16];
    assert!(FernetKey::from_bytes(&short_key).is_err(), "Short key should be rejected");
    
    let long_key = [42u8; 64];
    assert!(FernetKey::from_bytes(&long_key).is_err(), "Long key should be rejected");
}
...
async fn test_config_from_environment() {
    ...
    assert_eq!(config.log_level, fernet_web::server::config::LogLevel::from(tracing::Level::DEBUG));
...
async fn test_config_serialization() {
    ...
    let deserialized: ServerConfig = serde_json::from_str(&json).expect("Should deserialize from JSON");
    assert_eq!(deserialized.bind_addr.port(), original_config.bind_addr.port());
    assert_eq!(deserialized.log_level, original_config.log_level);
    assert_eq!(deserialized.max_payload_size, original_config.max_payload_size);
}
...
async fn test_crypto_operations_basic() {
    ...
    let result = crypto_service.decrypt_symmetric_key("invalid_base64!").await;
    ...
    let result = crypto_service.decrypt_payload(&valid_key, "invalid_token").await;
...
async fn test_performance_basic() {
    ...
    let _service = CryptoService::new(key_file.path()).await.expect("Should create service");
    let elapsed = start.elapsed();
    
    assert!(elapsed < Duration::from_millis(100), 
           "Crypto service creation should be fast: {:?}", elapsed);
    ...
    assert!(elapsed < Duration::from_millis(10),
           "Key validation should be fast: {:?}", elapsed);
}
...
async fn test_concurrent_crypto_operations() {
    ...
    let crypto_service = std::sync::Arc::new(
        CryptoService::new(key_file.path()).await.expect("Should create crypto service")
    );
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let result = service.decrypt_symmetric_key(&format!("test_key_{}", i)).await;
    ...
    assert!(result.is_ok(), "Service should remain functional after concurrent access");
}
...
async fn test_error_conversions() {
    ...
    assert!(fernet_error.internal_message().contains("JSON parsing error"));
}
...
async fn test_edge_cases() {
    ...
    assert!(config.get_worker_threads() > 0, "Should always return positive thread count");
    
    // Test with zero values (should fail validation)
    config.max_payload_size = 0;
    assert!(config.validate().is_err(), "Zero payload size should fail validation");
    
    config.max_payload_size = 1024;
    config.request_timeout_ms = 0;
    assert!(config.validate().is_err(), "Zero timeout should fail validation");
}
...
async fn test_operation_timeouts() {
    ...
    assert!(result.unwrap().is_err(), "Invalid input should cause operation to fail");
}
